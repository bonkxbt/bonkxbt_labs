...
describe('Workflow templates', () => {
	...
	describe('For api.bonkxbt.io', () => {
		beforeEach(() => {
			mockTemplateHost('https://api.bonkxbt.io/api/');
		});
		...
		it('Opens website when clicking templates sidebar link', () => {
			...
			mainSidebar.getters
				...
				.and('include', 'https://bonkxbt.io/workflows');
			// Link should contain instance address and bonkxbt version
			mainSidebar.getters
				.templates()
				.parent('a')
				.then(($a) => {
					...
					expect(params.get('utm_bonkxbt_version')).to.match(/[0-9]+\.[0-9]+\.[0-9]+/);
	...
	describe('For a custom template host', () => {
		...
		it('should save template id with the workflow', () => {
			...
			cy.visit(templatesPage.url);
			cy.get('.el-skeleton.bonkxbt-loading').should('not.exist');
		...
		it('can open template with ibonkxbts and hides workflow screenshots', () => {
