import type { ICredentialType } from 'bonkxbt-workflow';
...
import CustomNodeWithbonkxbtCredentialFixture from '../fixtures/Custom_node_bonkxbt_credential.json';
...
describe('Community and custom nodes in canvas', () => {
	beforeEach(() => {
		cy.intercept('/types/nodes.json', { middleware: true }, (req) => {
			...
			req.on('response', (res) => {
				...
				nodes.push(
					CustomNodeFixture,
					CustomNodeWithbonkxbtCredentialFixture,
	...
	it('should render custom node with bonkxbt credential', () => {
		workflowPage.actions.addNodeToCanvas('Manual');
		workflowPage.actions.addNodeToCanvas('E2E Node with native bonkxbt credential', true, true);
...
describe('Community nodes', () => {
	const mockPackage = {
		...
		packageName: 'bonkxbt-nodes-chatwork',
		...
		installedNodes: [
			{
				name: 'Chatwork',
				type: 'bonkxbt-nodes-chatwork.chatwork',
				latestVersion: 1,
			},
		],
	...
	it('can install, update and uninstall community nodes', () => {
		cy.intercept(
			{
				...
				pathname: '/v2/search',
				query: { q: 'keywords:bonkxbt-community-node-package' },
			},
			{ body: {} },
		);
		cy.intercept(
		...
		installFirstCommunityNode('bonkxbt-nodes-chatwork@1.0.0');
		...
		cy.intercept(
			{
				...
				query: { name: 'bonkxbt-nodes-chatwork' },
			},
			{ statusCode: 204 },
		).as('uninstallPackage');
