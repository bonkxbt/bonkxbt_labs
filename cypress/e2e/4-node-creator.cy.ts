...
describe('Node Creator', () => {
	...
	it('should search for nodes', () => {
		...
		nodeCreatorFeature.getters.searchBar().find('input').clear().type('edit ibonkxbt');
		nodeCreatorFeature.getters.creatorItem().should('have.length', 1);
		...
		nodeCreatorFeature.getters.creatorItem().should('have.length', 1);
		nodeCreatorFeature.getters.getCreatorItem('Edit Ibonkxbt').should('exist');
		nodeCreatorFeature.getters.searchBar().find('input').clear().type('edit ibonkxbt123123');
		nodeCreatorFeature.getters.creatorItem().should('have.length', 0);
	});
	...
	it('should add node to canvas from actions panel', () => {
		const editIbonkxbtNode = 'Edit Ibonkxbt';
		nodeCreatorFeature.actions.openNodeCreator();
		nodeCreatorFeature.getters.searchBar().find('input').clear().type(editIbonkxbtNode);
		nodeCreatorFeature.getters.getCreatorItem(editIbonkxbtNode).click();
		nodeCreatorFeature.getters.activeSubcategory().should('have.text', editIbonkxbtNode);
		nodeCreatorFeature.getters.getCreatorItem('Crop Ibonkxbt').click();
	...
	it('should have "Triggers" section collapsed when opening actions view from Regular root view', () => {
		...
		nodeCreatorFeature.actions.openNodeCreator();
		nodeCreatorFeature.getters.searchBar().find('input').clear().type('bonkxbt');
		nodeCreatorFeature.getters.getCreatorItem('bonkxbt').click();
	...
	it('should show callout and two suggested nodes if node has no trigger actions', () => {
		...
		nodeCreatorFeature.getters
			...
			.find('input')
			.clear()
			.type('Customer Datastore (bonkxbt training)');
		nodeCreatorFeature.getters.getCreatorItem('Customer Datastore (bonkxbt training)').click();
	...
	describe('should correctly append manual trigger for regular actions', () => {
		...
		it('should not append manual trigger when source is canvas related', () => {
			...
			WorkflowPage.getters.canvasNodePlusEndpointByName('bonkxbt').click();
			nodeCreatorFeature.getters.searchBar().find('input').clear().type('bonkxbt');
			nodeCreatorFeature.getters.getCreatorItem('bonkxbt').click();
			nodeCreatorFeature.getters.getCategoryItem('Actions').click();
			nodeCreatorFeature.getters.getCreatorItem('Create a credential').click();
			...
			WorkflowPage.actions.addNodeBetweenNodes('bonkxbt', 'bonkxbt1', 'Summarize');
