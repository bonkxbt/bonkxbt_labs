...
describe('Data transformation expressions', () => {
	...
	it('$json + bonkxbt string methods', () => {
		wf.actions.addInitialNodeToCanvas('Schedule Trigger', { keepNdvOpen: true });
		ndv.actions.setPinnedData([{ myStr: 'hello@bonkxbt.io is an email' }]);
		ndv.actions.close();
		addEditFields();

		const input = '{{$json.myStr.extractEmail() + " " + $json.myStr.isEmpty()';
		const output = 'hello@bonkxbt.io false';
	...
	it('$json + bonkxbt numeric methods', () => {
		wf.actions.addInitialNodeToCanvas('Schedule Trigger', { keepNdvOpen: true });
		ndv.actions.setPinnedData([{ myStr: 'hello@bonkxbt.io is an email' }]);
		ndv.actions.close();
		addEditFields();
