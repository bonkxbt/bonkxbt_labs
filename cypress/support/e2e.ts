...
beforeEach(() => {
	...
	cy.window().then((win): void => {
		win.localStorage.setItem('bonkxbt_THEME', 'light');
		win.localStorage.setItem('bonkxbt_AUTOCOMPLETE_ONBOARDED', 'true');
		win.localStorage.setItem('bonkxbt_MAPPING_ONBOARDED', 'true');
	});
	...
	cy.intercept({ pathname: '/api/versions/*' }, [
		{
			...
			documentationUrl: 'https://docs.bonkxbt.io/release-notes/#bonkxbt131',
			nodes: [],
		...
		},
		{
			...
			documentationUrl: 'https://docs.bonkxbt.io/release-notes/#bonkxbt104',
			nodes: [],
