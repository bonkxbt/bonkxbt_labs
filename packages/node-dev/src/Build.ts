...
import { InstanceSettings } from 'bonkxbt-core';
import { jsonParse } from 'bonkxbt-workflow';
...
export async function createCustomTsconfig() {
	...
	const tsconfigPath = join(dirname(require.resolve('bonkxbt-node-dev/src')), 'tsconfig-build.json');
...
export async function buildFiles({
	...
	// Supply a node base path so that it finds bonkxbt-core and bonkxbt-workflow
