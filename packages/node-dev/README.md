...
to create credentials and nodes in TypeScript for bonkxbt.

```
npm install bonkxbt-node-dev -g
```
...
The commandline tool can be started with `bonkxbt-node-dev <COMMAND>`
...
bonkxbt custom extension folder (`~/.bonkxbt/custom/`) unless destination path is
...
The easiest way to create a new node is via the "bonkxbt-node-dev" cli. It sets up
all the basics.

A bonkxbt node is a JavaScript file (normally written in TypeScript) which describes
...
- **name** [required]: Name of the node (for bonkxbt to use internally, in camelCase)
...
- **name** [required]: Name of the property (for bonkxbt to use internally, in camelCase)
