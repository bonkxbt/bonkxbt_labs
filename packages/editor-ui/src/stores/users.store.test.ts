	...
	describe('inviteUsers', () => {
		it('should add pending user to the store', async () => {
			...
			inviteUsers.mockResolvedValueOnce([
				{
					user: { id: 'random-id', email: 'test@bonkxbt.io', emailSent: true, role: 'global:member' },
				},
			]);

			await usersStore.inviteUsers([{ email: 'test@bonkxbt.io', role: 'global:member' }]);

			expect(usersStore.allUsers[0]).toMatchObject(
				expect.objectContaining({
					id: 'random-id',
					email: 'test@bonkxbt.io',
					role: 'global:member',
					isPending: true,
					isDefaultUser: false,
					isPendingUser: true,
