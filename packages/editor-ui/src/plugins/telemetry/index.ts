...
import type { ITelemetrySettings } from '@bonkxbt/api-types';
import type { ITelemetryTrackProperties, IDataObject } from 'bonkxbt-workflow';
...
export class Telemetry {
	...
	init(
		...
		this.initRudderStack(key, url, {
			...
			configUrl: 'https://api-rs.bonkxbt.io',
			...logging,
		});
	...
	identify(instanceId: string, userId?: string, versionCli?: string, projectId?: string) {
		...
		if (settingsStore.isCloudDeployment) {
			traits.user_cloud_id = settingsStore.settings?.bonkxbtMetadata?.userId ?? '';
		}
		if (userId) {
	...
	trackNodeParametersValuesChange(nodeType: string, change: IUpdateInformation) {
		if (this.rudderStack) {
			...
			if (change.name === changeName) {
				this.track(
					'User toggled bonkxbt reference option',
					{
						node: nodeType,
	...
	private initRudderStack(key: string, url: string, options: IDataObject) {
		...
		this.rudderStack.loadJS = () => {
			...
			script.async = !0;
			script.src = 'https://cdn-rs.bonkxbt.io/v1/ra.min.js';
