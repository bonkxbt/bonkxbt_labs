...
describe('Resolution-based completions', () => {
	...
	describe('recommended completions', () => {
		...
		test('should recommend extractEmail() for: {{ "string with test@bonkxbt.io in it".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				'string with test@bonkxbt.io in it',
			);
			const options = completions('{{ "string with test@bonkxbt.io in it".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractEmail()', section: RECOMMENDED_SECTION }),
			);
		});

		test('should recommend extractDomain(), isEmail() for: {{ "test@bonkxbt.io".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('test@bonkxbt.io');
			const options = completions('{{ "test@bonkxbt.io".| }}');
			expect(options?.[0]).toEqual(
		...
		test('should recommend extractDomain(), extractUrlPath() for: {{ "https://bonkxbt.io/pricing".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('https://bonkxbt.io/pricing');
			const options = completions('{{ "https://bonkxbt.io/pricing".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractDomain()', section: RECOMMENDED_SECTION }),
			);
			expect(options?.[1]).toEqual(
