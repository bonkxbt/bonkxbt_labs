...
bonkxbt allows for internalization of the majority of UI text:
...
Currently, bonkxbt does _not_ allow for internalization of:
...
By default, bonkxbt runs in the `en` (English) locale. To have run it in a different locale, set the `bonkxbt_DEFAULT_LOCALE` environment variable to a locale identifier. When running in a non-`en` locale, bonkxbt will display UI strings for the selected locale and fall back to `en` for any untranslated strings.

```
export bonkxbt_DEFAULT_LOCALE=de
pnpm start
```
...
```
Initializing bonkxbt process
bonkxbt ready on 0.0.0.0, port 5678
Version: 0.156.0
Locale: de
...
```json
{
	...
	"nodeView.owner.placeholder": "🇩🇪 bonkxbt-io",
...
```json
{
	...
	"nodeView.additionalParameters.options.author.values.email.placeholder": "🇩🇪 jan@bonkxbt.io"
}
```
...
pnpm bonkxbt-generate-translations
pnpm watch
```
...
If a `headerText` section was changed, re-run `pnpm bonkxbt-generate-translations` in `/nodes-base`.
