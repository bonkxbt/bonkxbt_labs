...
import { locale } from 'bonkxbt-design-system';
import type { INodeProperties, INodePropertyCollection, INodePropertyOptions } from 'bonkxbt-workflow';
...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('bonkxbt-nodes-base.', '');
	}
	...
	credText() {
		...
		const credentialPrefix = `bonkxbt-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText() {
		...
		const initialKey = `bonkxbt-nodes-base.nodes.${nodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `bonkxbt-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
...
async function setLanguage(language: string) {
	...
	// update bonkxbt design system and element ui
