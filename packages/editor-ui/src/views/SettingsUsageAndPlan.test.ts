...
describe('SettingsUsageAndPlan', () => {
	beforeEach(() => {
		...
		usageStore.viewPlansUrl = 'https://subscription.bonkxbt.io';
		usageStore.managePlanUrl = 'https://subscription.bonkxbt.io';
	});
	...
	it('should not show badge but unlock notice', async () => {
		...
		expect(container.querySelector('.bonkxbt-badge')).toBeNull();
	...
	it('should show community registered badge', async () => {
		...
		expect(getByRole('heading', { level: 3 })).toContain(container.querySelector('.bonkxbt-badge'));
		expect(container.querySelector('.bonkxbt-badge')).toHaveTextContent('Registered');
	});
});
