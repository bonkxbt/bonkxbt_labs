...
import { createFormEventBus } from 'bonkxbt-design-system/utils';
...
<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		<div :class="$style.header">
			...
			<div v-if="currentUser" :class="$style.user">
				<span :class="$style.username" data-test-id="current-user-name">
					<bonkxbt-text color="text-light">{{ currentUser.fullName }}</bonkxbt-text>
				</span>
				<bonkxbt-avatar
		...
		<div v-if="isPersonalSecurityEnabled">
			<div class="mb-s">
				<bonkxbt-heading size="large">{{ i18n.baseText('settings.personal.security') }}</bonkxbt-heading>
			</div>
			<div class="mb-s">
				<bonkxbt-input-label :label="i18n.baseText('auth.password')">
					<bonkxbt-link data-test-id="change-password-link" @click="openPasswordModal">{{
			...
			<div v-if="isMfaFeatureEnabled" data-test-id="mfa-section">
				<div class="mb-xs">
					<bonkxbt-input-label :label="i18n.baseText('settings.personal.mfa.section.title')" />
					<bonkxbt-text :bold="false" :class="$style.infoText">
						{{
							mfaDisabled
								? i18n.baseText('settings.personal.mfa.button.disabled.infobox')
								: i18n.baseText('settings.personal.mfa.button.enabled.infobox')
						}}
						<bonkxbt-link :to="MFA_DOCS_URL" size="small" :bold="true">
		...
		<div>
			...
			<div>
				<bonkxbt-input-label :label="i18n.baseText('settings.personal.theme')">
					<bonkxbt-select
