...
import bonkxbtHeading from 'bonkxbt-design-system/components/bonkxbtHeading';
import bonkxbtLink from 'bonkxbt-design-system/components/bonkxbtLink';
...
<template>
	<TemplatesView :go-back-enabled="true">
		<template #header>
			<bonkxbtHeading v-if="isReady" tag="h1" size="2xlarge"
				>{{ i18n.baseText('templateSetup.title', { interpolate: { name: title } }) }}
			</bonkxbtHeading>
			<bonkxbt-loading v-else variant="h1" />
		</template>
		...
		<template #content>
			<div :class="$style.grid">
				<div :class="$style.notice" data-test-id="info-callout">
					...
					/>
					<bonkxbt-loading v-else variant="p" />
				</div>
				...
				<div>
					...
					<div v-else :class="$style.appCredentialsContainer">
						<bonkxbt-loading :class="$style.appCredential" variant="p" :rows="3" />
						<bonkxbt-loading :class="$style.appCredential" variant="p" :rows="3" />
					</div>
				</div>

				<div :class="$style.actions">
					<bonkxbtLink :href="skipSetupUrl" :new-window="false" @click="onSkipSetup($event)">{{
					...
					<div v-else>
						<bonkxbt-loading variant="button" />
					</div>
				</div>
			</div>
		</template>
