...
import bonkxbtInfoTip from 'bonkxbt-design-system/components/bonkxbtInfoTip';
...
<template>
	<div class="settings-usage-and-plan">
		<bonkxbt-heading tag="h2" size="2xlarge">{{
			locale.baseText('settings.usageAndPlan.title')
		}}</bonkxbt-heading>
		<div v-if="!usageStore.isLoading">
			<bonkxbt-heading tag="h3" :class="$style.title" size="large">
				...
				<span v-if="badgedPlanName.badge && badgedPlanName.name" :class="$style.titleTooltip">
					<bonkxbtTooltip placement="top">
						...
						<bonkxbtBadge>{{ badgedPlanName.badge }}</bonkxbtBadge>
					</bonkxbtTooltip>
				</span>
			</bonkxbt-heading>

			<bonkxbtNotice v-if="isCommunity && canUserRegisterCommunityPlus" class="mt-0" theme="warning">
			...
			<div :class="$style.quota">
				<bonkxbt-text size="medium" color="text-light">
			...
			<bonkxbtInfoTip>{{ locale.baseText('settings.usageAndPlan.activeWorkflows.hint') }}</bonkxbtInfoTip>
			...
			<div :class="$style.buttons">
				...
				<bonkxbt-button v-if="usageStore.managementToken" size="large" @click="onManagePlan">
					<a :href="managePlanUrl" target="_blank">{{
						locale.baseText('settings.usageAndPlan.button.manage')
					}}</a>
				</bonkxbt-button>
				<bonkxbt-button v-else size="large" @click.prevent="onViewPlans">
			...
			>
				...
				<template #footer>
					<bonkxbt-button type="secondary" @click="activationKeyModal = false">
						{{ locale.baseText('settings.usageAndPlan.dialog.activation.cancel') }}
					</bonkxbt-button>
					<bonkxbt-button @click="onLicenseActivation">
