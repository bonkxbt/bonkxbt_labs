...
import type { Rule, RuleGroup } from 'bonkxbt-design-system/types';
...
<template>
	<div>
		<bonkxbt-heading size="2xlarge" tag="h1">{{
		...
		>
			<bonkxbt-callout theme="secondary" icon="info-circle" class="mt-2xl mb-l">
			...
			<bonkxbt-heading size="xlarge" tag="h2" class="mb-s">{{
			...
			<div :class="$style.group">
				...
				<div :class="$style.groupFlex">
					...
					<bonkxbt-button
						...
						@click="onDisconnect"
						>{{ locale.baseText('settings.sourceControl.button.disconnect') }}</bonkxbt-button
					>
				</div>
			</div>
			<div v-if="sourceControlStore.preferences.publicKey" :class="$style.group">
				...
				<bonkxbt-notice type="info" class="mt-s">
			...
			<bonkxbt-button
				...
				@click="onConnect"
				>{{ locale.baseText('settings.sourceControl.button.connect') }}</bonkxbt-button
			>
			<div v-if="isConnected" data-test-id="source-control-connected-content">
				...
				<div :class="$style.group">
					...
					<div>
						<bonkxbt-color-picker v-model="sourceControlStore.preferences.branchColor" size="small" />
					</div>
				</div>
				<div :class="[$style.group, 'pt-s']">
					<bonkxbt-button
						...
						@click="onSave"
						>{{ locale.baseText('settings.sourceControl.button.save') }}</bonkxbt-button
					>
				</div>
			</div>
		</div>
