		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<bonkxbt-heading v-if="collection && collection.name" tag="h1" size="2xlarge">
						{{ collection.name }}
					</bonkxbt-heading>
					<bonkxbt-text v-if="collection && collection.name" color="text-base" size="small">
						{{ i18n.baseText('templates.collection') }}
					</bonkxbt-text>
					<bonkxbt-loading :loading="!collection || !collection.name" :rows="2" variant="h1" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<bonkxbt-text color="text-base">{{ i18n.baseText('templates.collectionsNotFound') }}</bonkxbt-text>
			</div>
		</template>
				...
				<div :class="$style.mainContent">
					<div v-if="loading || isFullTemplatesCollection(collection)" :class="$style.markdown">
						<bonkxbt-markdown
							...
							:ibonkxbts="isFullTemplatesCollection(collection) && collection.ibonkxbt"
							:loading="loading"
						/>
					</div>
					<TemplateList
