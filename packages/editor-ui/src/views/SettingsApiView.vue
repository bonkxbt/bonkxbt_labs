...
function onUpgrade() {
	void goToUpgrade('settings-bonkxbt-api', 'upgrade-api', 'redirect');
}
...
<template>
	<div :class="$style.container">
		...
		<div v-if="apiKeys.length">
			<p class="mb-s">
				<bonkxbt-info-tip :bold="false">
					<i18n-t keypath="settings.api.view.info" tag="span">
						<template #apiAction>
							<a
								href="https://docs.bonkxbt.io/api"
								target="_blank"
						...
						<template #webhookAction>
							<a
								href="https://docs.bonkxbt.io/integrations/core-nodes/bonkxbt-nodes-base.webhook/"
								target="_blank"
			...
			<bonkxbt-card class="mb-4xs" :class="$style.card">
				<span :class="$style.delete">
					<bonkxbt-link :bold="true" @click="showDeleteModal">
			...
			<div :class="$style.hint">
				<bonkxbt-text size="small">
					{{ i18n.baseText(`settings.api.view.${isSwaggerUIEnabled ? 'tryapi' : 'more-details'}`) }}
				</bonkxbt-text>
				{{ ' ' }}
				<bonkxbt-link :to="apiDocsURL" :new-window="true" size="small">
					{{
						i18n.baseText(
