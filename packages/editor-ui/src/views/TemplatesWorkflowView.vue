		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<bonkxbt-heading v-if="template && template.name" tag="h1" size="2xlarge">{{
						template.name
					}}</bonkxbt-heading>
					<bonkxbt-text v-if="template && template.name" color="text-base" size="small">
						{{ i18n.baseText('generic.workflow') }}
					</bonkxbt-text>
					<bonkxbt-loading :loading="!template || !template.name" :rows="2" variant="h1" />
				</div>
				<div :class="$style.button">
					...
					/>
					<bonkxbt-loading :loading="!template" :rows="1" variant="button" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<bonkxbt-text color="text-base">{{ i18n.baseText('templates.workflowsNotFound') }}</bonkxbt-text>
			</div>
		</template>
		<template v-if="!notFoundError" #content>
			<div :class="$style.ibonkxbt">
				<WorkflowPreview
					v-if="showPreview"
					:loading="loading"
