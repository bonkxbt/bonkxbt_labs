...
import type { bonkxbtPlusEndpoint } from '@/plugins/jsplumb/bonkxbtPlusEndpointType';
import {
	bonkxbtPlusEndpointType,
	EVENT_PLUS_ENDPOINT_CLICK,
} from '@/plugins/jsplumb/bonkxbtPlusEndpointType';
import type { bonkxbtAddInputEndpoint } from '@/plugins/jsplumb/bonkxbtAddInputEndpointType';
import {
	EVENT_ADD_INPUT_ENDPOINT_CLICK,
	bonkxbtAddInputEndpointType,
} from '@/plugins/jsplumb/bonkxbtAddInputEndpointType';
...
export default defineComponent({
			...
			try {
				...
				if (window.parent) {
					window.parent.postMessage(
						JSON.stringify({ command: 'bonkxbtReady', version: this.rootStore.versionCli }),
						'*',
					);
				}
			} catch (error) {
	...
	methods: {
		...
		resetEndpointsErrors() {
			...
			allEndpoints
				.filter((endpoint) => endpoint?.endpoint.type === bonkxbtAddInputEndpointType)
				.forEach((endpoint) => {
					const bonkxbtAddInputEndpoint = endpoint?.endpoint as bonkxbtAddInputEndpoint;
					if (bonkxbtAddInputEndpoint && (endpoint?.connections ?? []).length > 0) {
						bonkxbtAddInputEndpoint.resetError();
					}
				});
		},
			...
			nodeIssues.forEach((node) => {
				...
				Object.keys(node?.issues?.input ?? {}).forEach((connectionType) => {
					...
					inputEndpointsWithIssues.forEach((endpoint) => {
						const bonkxbtAddInputEndpoint = endpoint?.endpoint as bonkxbtAddInputEndpoint;
						if (bonkxbtAddInputEndpoint) {
							bonkxbtAddInputEndpoint.setError();
						}
					});
				});
			});
		...
		onConnectionDragAbortDetached() {
			Object.values(this.instance?.endpointsByElement)
				...
				.filter((endpoint) => endpoint.endpoint.type === 'bonkxbtPlus')
			...
			if (this.instance) {
				...
				for (const element of Object.values(elements)) {
					...
					for (const endpoint of endpoints || []) {
						...
						if (endpointInstance && endpointInstance.type === bonkxbtPlusEndpointType) {
							(endpointInstance as bonkxbtPlusEndpoint).unbindEvents();
						}
					}
				}
			}
		...
		onNodeRun({
			...
			if (data === null || data.length === 0 || waiting) {
				...
				endpoints.forEach((endpoint: Endpoint) => {
					if (endpoint.endpoint.type === 'bonkxbtPlus') {
						(endpoint.endpoint as bonkxbtPlusEndpoint).clearSuccessOutput();
					}
				});
