...
import { TelemetryHelpers, NodeConnectionType, jsonParse } from 'bonkxbt-workflow';
import type { IDataObject, ExecutionSummary, IConnection, IWorkflowBase } from 'bonkxbt-workflow';
...
function emitPostMessageReady() {
	if (window.parent) {
		window.parent.postMessage(
			JSON.stringify({ command: 'bonkxbtReady', version: rootStore.versionCli }),
			'*',
		);
	}
}
