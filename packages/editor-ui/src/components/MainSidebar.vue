...
import { bonkxbtNavigationDropdown } from 'bonkxbt-design-system';
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				link: {
					href: 'https://docs.bonkxbt.io?utm_source=bonkxbt_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://community.bonkxbt.io?utm_source=bonkxbt_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://docs.bonkxbt.io/courses/',
					target: '_blank',
				},
			},
			{
			...
			},
			{
				id: 'about',
				icon: 'info',
				label: i18n.baseText('mainSidebar.aboutbonkxbt'),
				position: 'bottom',
			},
		],
	},
]);
const createBtn = ref<InstanceType<typeof bonkxbtNavigationDropdown>>();
...
<template>
	...
	>
		...
		>
			<bonkxbtIcon v-if="isCollapsed" icon="chevron-right" size="xsmall" class="ml-5xs" />
			<bonkxbtIcon v-else icon="chevron-left" size="xsmall" class="mr-5xs" />
		</div>
		<div :class="$style.logo">
			...
			/>
			<bonkxbtNavigationDropdown
				ref="createBtn"
				data-test-id="universal-add"
				:menu="menu"
				@select="handleMenuSelect"
			>
				<bonkxbtIconButton icon="plus" type="secondary" outline />
				<template #[createProjectAppendSlotName]="{ item }">
					<bonkxbtTooltip v-if="item.disabled" placement="right" :content="projectsLimitReachedMessage">
						<bonkxbtButton
							:size="'mini'"
		...
		<bonkxbtMenu :items="mainMenuItems" :collapsed="isCollapsed" @select="handleSelect">
			<template #header>
				<ProjectNavigation
					:collapsed="isCollapsed"
			...
			<template v-if="showUserArea" #footer>
				<div ref="user" :class="$style.userArea">
					...
					>
						<bonkxbtText size="small" :bold="true" color="text-dark">{{
							usersStore.currentUser?.fullName
						}}</bonkxbtText>
					</div>
					<div :class="{ [$style.userActions]: true, [$style.expanded]: fullyExpanded }">
						<bonkxbtActionDropdown
							:items="userMenuItems"
							placement="top-start"
