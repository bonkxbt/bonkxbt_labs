...
import { bonkxbtOption, bonkxbtSelect } from 'bonkxbt-design-system';
...
const selectRef = ref<InstanceType<typeof bonkxbtSelect>>();
const tagRefs = ref<Array<InstanceType<typeof bonkxbtOption>>>();
const createRef = ref<InstanceType<typeof bonkxbtOption>>();
...
<template>
	<div ref="container" :class="{ 'tags-container': true, focused }" @keydown.stop>
		...
		>
			...
			<bonkxbtOption v-else-if="options.length === 0" value="message" disabled>
			...
			<bonkxbtOption :key="MANAGE_KEY" :value="MANAGE_KEY" class="ops manage-tags">
