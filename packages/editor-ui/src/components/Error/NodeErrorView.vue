...
import InlineAskAssistantButton from 'bonkxbt-design-system/components/InlineAskAssistantButton/InlineAskAssistantButton.vue';
...
function copyErrorDetails() {
	...
	//add bonkxbt details
	const bonkxbtDetails: IDataObject = {};

	if (error.node) {
		bonkxbtDetails.nodeName = error.node.name;
		bonkxbtDetails.nodeType = error.node.type;
		bonkxbtDetails.nodeVersion = error.node.typeVersion;

		if (error.node?.parameters?.resource) {
			bonkxbtDetails.resource = error.node.parameters.resource;
		}
		if (error?.node?.parameters?.operation) {
			bonkxbtDetails.operation = error.node.parameters.operation;
		}
	}

	if (error.context) {
		if (error.context.itemIndex !== undefined) {
			bonkxbtDetails.itemIndex = error.context.itemIndex;
		}

		if (error.context.runIndex !== undefined) {
			bonkxbtDetails.runIndex = error.context.runIndex;
		}

		if (error.context.parameter !== undefined) {
			bonkxbtDetails.parameter = error.context.parameter;
		}

		if (error.context.causeDetailed) {
			bonkxbtDetails.causeDetailed = error.context.causeDetailed;
		}
	}

	if (error.timestamp) {
		bonkxbtDetails.time = new Date(error.timestamp).toLocaleString();
	}

	bonkxbtDetails.bonkxbtVersion = bonkxbtVersion.value;

	bonkxbtDetails.binaryDataMode = rootStore.binaryDataMode;

	if (error.cause) {
		bonkxbtDetails.cause = error.cause;
	}

	bonkxbtDetails.stackTrace = error.stack?.split('\n');

	errorInfo.bonkxbtDetails = bonkxbtDetails;
	...
	<div class="node-error-view">
		<div class="node-error-view__header">
			...
			<div
				...
				v-bonkxbt-html="getErrorDescription()"
			></div>
				...
				<details class="node-error-view__details">
					...
					<div class="node-error-view__details-content">
						...
						<div class="node-error-view__details-row">
							<p class="node-error-view__details-label">
								{{ i18n.baseText('nodeErrorView.details.bonkxbtVersion') }}
							</p>
							<p class="node-error-view__details-value">
								<code>{{ bonkxbtVersion }}</code>
							</p>
						</div>
