...
import { createEventBus, createFormEventBus } from 'bonkxbt-design-system/utils';
...
const survey = computed<IFormInputs>(() => [
	{
		...
		properties: {
			...
			options: [
				...
				{
					label: i18n.baseText('personalizationModal.imNotUsingbonkxbtForWork'),
	...
	},
	{
		...
		properties: {
			type: 'select',
			label: i18n.baseText('personalizationModal.howDidYouHearAboutbonkxbt'),
			placeholder: i18n.baseText('personalizationModal.select'),
			options: [
				{
					label: 'Google',
...
const onSubmit = async (values: IPersonalizationLatestVersion) => {
	...
	try {
		const completedSurvey: IPersonalizationLatestVersion = {
			...
			personalization_survey_bonkxbt_version: rootStore.versionCli,
		};
...
<template>
	<Modal
		...
		:title="i18n.baseText('personalizationModal.customizebonkxbt')"
