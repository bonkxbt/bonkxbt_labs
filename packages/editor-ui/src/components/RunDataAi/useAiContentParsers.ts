import type { IDataObject, INodeExecutionData } from 'bonkxbt-workflow';
import { isObjectEmpty, NodeConnectionType } from 'bonkxbt-workflow';
					...
					if (
						...
						if (Array.isArray(message)) {
							message = (message as MessageContent[])
								.map((item) => {
									const { type, ibonkxbt_url } = item;
									if (
										type === 'ibonkxbt_url' &&
										typeof ibonkxbt_url === 'object' &&
										typeof ibonkxbt_url.url === 'string'
									) {
										return `![Input ibonkxbt](${ibonkxbt_url.url})`;
									} else if (typeof ibonkxbt_url === 'string') {
										return `![Input ibonkxbt](${ibonkxbt_url})`;
									}
									return item.text;
								})
								.join('\n');
