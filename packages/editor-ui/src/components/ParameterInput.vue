...
import { CREDENTIAL_EMPTY_VALUE, NodeHelpers } from 'bonkxbt-workflow';
...
import { bonkxbtIcon, bonkxbtInput, bonkxbtInputNumber, bonkxbtOption, bonkxbtSelect } from 'bonkxbt-design-system';
...
const inputField = ref<InstanceType<typeof bonkxbtInput | typeof bonkxbtSelect> | HTMLElement>();
...
function trackExpressionEditOpen() {
	...
	if (node.value.type.startsWith('bonkxbt-nodes-base') || isCredentialOnlyNodeType(node.value.type)) {
...
onMounted(() => {
	...
	void externalHooks.run('parameterInput.mount', {
		...
		inputFieldRef: inputField.value as InstanceType<typeof bonkxbtInput>,
	});
});
				...
				>
					<div class="list-option">
						...
						<div
							...
							v-bonkxbt-html="getOptionsOptionDescription(option)"
						></div>
					</div>
				</bonkxbtOption>
			</bonkxbtSelect>
