...
import type { CodeExecutionMode, CodeNodeEditorLanguage } from 'bonkxbt-workflow';
	...
	function lintSource(editorView: EditorView): Diagnostic[] {
		...
		try {
			...
			try {
				...
				/**
				 * For invalid (e.g. half-written) bonkxbt syntax, esprima errors with an off-by-one line number for the final line. In future, we should add full linting for bonkxbt syntax before parsing JS.
				 */
				return [];
			}
		}
		...
		if (toValue(mode) === 'runOnceForAllItems') {
			...
			const isForOfStatementOverbonkxbtVar = (node: Node) =>
				...
				node.right.callee.object.name.startsWith('$'); // bonkxbt var, e.g $input

			const found = walk<TargetNode>(ast, isForOfStatementOverbonkxbtVar);
