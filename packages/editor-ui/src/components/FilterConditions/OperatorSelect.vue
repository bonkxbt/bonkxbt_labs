...
import type { FilterOperatorType } from 'bonkxbt-workflow';
...
<template>
	...
	>
		<template v-if="selectedGroupIcon" #prefix>
			<bonkxbt-icon :class="$style.icon" :icon="selectedGroupIcon" color="text-light" size="small" />
		</template>
					...
					<template #reference>
						...
						>
							<div :class="$style.groupTitle">
								<bonkxbt-icon v-if="group.icon" :icon="group.icon" :class="$style.icon" size="small" />
								<span>{{ i18n.baseText(group.name) }}</span>
							</div>
							<bonkxbt-icon icon="chevron-right" color="text-light" size="xsmall" />
						</div>
					</template>
					<div>
						<bonkxbt-option
