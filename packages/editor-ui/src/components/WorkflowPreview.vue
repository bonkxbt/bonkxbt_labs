...
<template>
	<div :class="$style.container">
		<div v-if="loaderType === 'ibonkxbt' && !showPreview" :class="$style.ibonkxbtLoader">
			<bonkxbt-loading :loading="!showPreview" :rows="1" variant="ibonkxbt" />
		</div>
		<div v-else-if="loaderType === 'spinner' && !showPreview" :class="$style.spinner">
			<bonkxbt-spinner type="dots" />
		</div>
		<iframe
			ref="iframeRef"
			:class="{
