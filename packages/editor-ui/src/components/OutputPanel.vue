...
import { bonkxbtRadioButtons, bonkxbtText } from 'bonkxbt-design-system';
...
<template>
	...
	>
		...
		<template #node-not-run>
			<bonkxbtText v-if="workflowRunning && !isTriggerNode" data-test-id="ndv-output-waiting">{{
				i18n.baseText('ndv.output.waitingToRun')
			}}</bonkxbtText>
			<bonkxbtText v-if="!workflowRunning" data-test-id="ndv-output-run-node-hint">
				...
				<template v-else>
					...
					<span v-if="canPinData" @click="insertTestData">
						...
						<bonkxbtText tag="a" size="medium" color="primary">
		...
		<template #node-waiting>
			...
			<bonkxbtText v-bonkxbt-html="waitingNodeTooltip(node)"></bonkxbtText>
		</template>

		<template #no-output-data>
			<bonkxbtText :bold="true" color="text-dark" size="large">{{
