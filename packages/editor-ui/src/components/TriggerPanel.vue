...
import type { INodeTypeDescription } from 'bonkxbt-workflow';
		...
		<transition name="fade" mode="out-in">
			...
			<div v-else-if="isListeningForEvents" key="listening">
				...
				<div v-if="isWebhookNode">
					<bonkxbt-text tag="div" size="large" color="text-dark" class="mb-2xs" bold>{{
						i18n.baseText('ndv.trigger.webhookNode.listening')
					}}</bonkxbt-text>
				...
				<div v-else>
					...
					<div :class="[$style.shake, 'mb-xs']">
						<bonkxbt-text tag="div">
							{{ listeningHint }}
						</bonkxbt-text>
					</div>
					<div v-if="displayChatButton">
						<bonkxbt-button class="mb-xl" @click="openWebhookUrl()">
			...
			<div v-else key="default">
				<div v-if="isActivelyPolling" class="mb-xl">
					<bonkxbt-spinner type="ring" />
				</div>

				<div :class="$style.action">
					<div :class="$style.header">
						<bonkxbt-heading v-if="header" tag="h1" bold>
							{{ header }}
						</bonkxbt-heading>
						<bonkxbt-text v-if="subheader">
				...
				<bonkxbt-text v-if="activationHint" size="small" @click="onLinkClick">
					<span v-bonkxbt-html="activationHint"></span>&nbsp;
				</bonkxbt-text>
				<bonkxbt-link
					v-if="activationHint && executionsHelp"
					size="small"
					@click="expandExecutionHelp"
					>{{ i18n.baseText('ndv.trigger.moreInfo') }}</bonkxbt-link
				>
				<bonkxbt-info-accordion
					v-if="executionsHelp"
					ref="help"
