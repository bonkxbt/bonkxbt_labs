...
onBeforeMount(async () => {
	...
	const key = `bonkxbt-nodes-base.credentials.${props.credentialType.name}`;
...
const documentationUrl = computed(() => {
	...
	if (url.hostname === DOCS_DOMAIN) {
		url.searchParams.set('utm_source', 'bonkxbt_app');
		...
		<div :class="$style.config" data-test-id="node-credentials-config-container">
			...
			<template v-if="credentialPermissions.update">
				...
				>
					...
					<span class="ml-4xs">
						<bonkxbt-link :to="documentationUrl" size="small" bold @click="onDocumentationUrlClick">
			...
			<bonkxbt-text v-if="isMissingCredentials" color="text-base" size="medium">
			...
			<EnterpriseEdition :features="[EnterpriseEditionFeature.ExternalSecrets]">
				<template #fallback>
					<bonkxbt-info-tip class="mt-s">
						{{ i18n.baseText('credentialEdit.credentialConfig.externalSecrets') }}
						<bonkxbt-link bold :to="i18n.baseText('settings.externalSecrets.docs')" size="small">
