...
import { bonkxbtIconButton, useDeviceSupport } from 'bonkxbt-design-system';
		...
		>
			...
			>
				<i v-if="isTriggerNode" class="trigger-icon">
					<bonkxbt-tooltip placement="bottom">
						<template #content>
							<span v-bonkxbt-html="i18n.baseText('node.thisIsATriggerNode')" />
						</template>
				...
				>
					<div v-if="hasIssues && !hideNodeIssues" class="node-issues" data-test-id="node-issues">
						<bonkxbt-tooltip :show-after="500" placement="bottom">
							<template #content>
...
.node-wrapper {
	...
	*/
	...
	&--configurable {
		...
		&[data-node-type='@bonkxbt/bonkxbt-nodes-langchain.chatTrigger'] {
