...
import { deepCopy, ADD_FORM_NOTICE } from 'bonkxbt-workflow';
...
import { bonkxbtNotice, bonkxbtIconButton, bonkxbtInputLabel, bonkxbtText, bonkxbtIcon } from 'bonkxbt-design-system';
		...
		>
			...
			>
				...
				<Suspense v-if="!asyncLoadingError">
					...
					<template #fallback>
						<bonkxbtText size="small" class="async-notice">
							<bonkxbtIcon icon="sync-alt" size="xsmall" :spin="true" />
							{{ i18n.baseText('parameterInputList.loadingFields') }}
						</bonkxbtText>
					</template>
				</Suspense>
				<bonkxbtText v-else size="small" color="danger" class="async-notice">
