...
const commonCode = computed(() => ({
	import: `${importCode} '@bonkxbt/chat/style.css';
${importCode} { createChat } from '@bonkxbt/chat';`,
	createChat: `createChat({
	webhookUrl: '${webhookUrl.value}'
});`,
	install: 'npm install @bonkxbt/chat',
}));

const cdnCode = computed(
	() => `<link href="https://cdn.jsdelivr.net/npm/@bonkxbt/chat/dist/style.css" rel="stylesheet" />
<script type="module">
${importCode} { createChat } from 'https://cdn.jsdelivr.net/npm/@bonkxbt/chat/dist/chat.bundle.es.js';
	...
	>
		<template #content>
			<div :class="$style.container">
				<bonkxbt-tabs v-model="currentTab" :options="tabs" />
				...
				<bonkxbt-text>
					...
					<bonkxbt-link :href="i18n.baseText('chatEmbed.url')" new-window bold>
		...
		<template #footer>
			<div class="action-buttons">
				<bonkxbt-button float="right" :label="i18n.baseText('chatEmbed.close')" @click="closeDialog" />
			</div>
		</template>
	</Modal>
</template>
