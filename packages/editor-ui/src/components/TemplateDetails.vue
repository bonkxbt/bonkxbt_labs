...
<template>
	<div>
		<bonkxbt-loading :loading="loading" :rows="5" variant="p" />
		...
		>
			<bonkxbt-tags :tags="template.categories" @click:tag="redirectToCategory" />
		</TemplateDetailsBlock>
		...
		>
			<div :class="$style.text">
				<bonkxbt-text v-if="isTemplatesWorkflow(template)" size="small" color="text-base">
					{{ i18n.baseText('template.details.created') }}
					<TimeAgo :date="template.createdAt" />
					{{ i18n.baseText('template.details.by') }}
					{{ template.user ? template.user.username : 'bonkxbt team' }}
				</bonkxbt-text>
			</div>
			<div :class="$style.text">
				<bonkxbt-text
