		...
		if (error && errorsToTrack.some((e) => error.message?.toLowerCase().includes(e))) {
			telemetry.track(
				'User encountered an error',
				{
					...
					bonkxbtVersion: rootStore.versionCli,
				},
				{
					withPostHog: true,
				},
...
function isViewable(index: number, key: string | number): boolean {
	...
	return (
		!!fileType && ['ibonkxbt', 'audio', 'video', 'text', 'json', 'pdf', 'html'].includes(fileType)
	);
}
	...
	<div :class="['run-data', $style.container]" @mouseover="activatePane">
		...
		>
			<div :class="$style.runSelectorInner">
				...
				<bonkxbtTooltip v-if="canLinkRuns" placement="right">
					<template #content>
			...
			>
				<bonkxbtIcon icon="external-link-alt" size="xsmall" />
		...
		<div v-if="props.calloutMessage" :class="$style.hintCallout">
			<bonkxbtCallout theme="secondary" data-test-id="run-data-callout">
				<bonkxbtText v-bonkxbt-html="props.calloutMessage" size="small"></bonkxbtText>
			</bonkxbtCallout>
		</div>
		...
		>
			<bonkxbtText v-bonkxbt-html="hint.message" size="small"></bonkxbtText>
		</bonkxbtCallout>
		...
		>
			...
			<bonkxbtText v-if="search" :class="$style.itemsText">
				{{
					i18n.baseText('ndv.search.items', {
			...
			<bonkxbtText v-else :class="$style.itemsText">
				<span>
					{{
						i18n.baseText('ndv.output.items', {
		...
		<div ref="dataContainerRef" :class="$style.dataContainer" data-test-id="ndv-data-container">
			...
			>
				<div :class="$style.spinner"><bonkxbtSpinner type="ring" /></div>
				<bonkxbtText>{{ executingMessage }}</bonkxbtText>
			</div>
			...
			<div v-else-if="editMode.enabled" :class="$style.editMode">
				...
				<div :class="$style.editModeFooter">
					<bonkxbtInfoTip :bold="false" :class="$style.editModeFooterInfotip">
						{{ i18n.baseText('runData.editor.copyDataInfo') }}
						<bonkxbtLink :to="DATA_EDITING_DOCS_URL" size="small">
			...
			<div v-else-if="hasNodeRun && hasRunError" :class="$style.stretchVertically">
				<bonkxbtText v-if="isPaneTypeInput" :class="$style.center" size="large" tag="p" bold>
					{{
			...
			>
				<div v-if="search">
					<bonkxbtText tag="h3" size="large">{{ i18n.baseText('ndv.search.noMatch.title') }}</bonkxbtText>
					<bonkxbtText>
			...
			<div
				...
				:class="$style.center"
			>
				<bonkxbtText :bold="true" color="text-dark" size="large">{{ tooMuchDataTitle }}</bonkxbtText>
				<bonkxbtText align="center" tag="div"
					><span
						v-bonkxbt-html="
			...
			<div v-else-if="displayMode === 'binary' && binaryData.length === 0" :class="$style.center">
				<bonkxbtText align="center" tag="div">{{ i18n.baseText('runData.noBinaryDataFound') }}</bonkxbtText>
			</div>
						...
						>
							<div :data-test-id="'ndv-binary-data_' + index">
								...
								<div v-if="binaryData.fileName">
									<div>
										<bonkxbtText size="small" :bold="true"
								...
								<div v-if="binaryData.fileExtension">
									<div>
										<bonkxbtText size="small" :bold="true"
											>{{ i18n.baseText('runData.fileExtension') }}:</bonkxbtText
										>
									</div>
		...
		>
			...
			<div :class="$style.pageSizeSelector">
				...
				>
					...
					<bonkxbtOption v-for="size in pageSizes" :key="size" :label="size" :value="size"> </bonkxbtOption>
					<bonkxbtOption :label="i18n.baseText('ndv.output.all')" :value="dataCount"> </bonkxbtOption>
				</bonkxbtSelect>
			</div>
		</div>
		<bonkxbtBlockUi :show="blockUI" :class="$style.uiBlocker" />
	</div>
</template>
