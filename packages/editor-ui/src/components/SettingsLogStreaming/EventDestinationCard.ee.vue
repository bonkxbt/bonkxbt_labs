...
import type { MessageEventBusDestinationOptions } from 'bonkxbt-workflow';
import { deepCopy, defaultMessageEventBusDestinationOptions } from 'bonkxbt-workflow';
import type { BaseTextKey } from '@/plugins/i18n';
import type { EventBus } from 'bonkxbt-design-system';
...
<template>
	<bonkxbt-card :class="$style.cardLink" data-test-id="destination-card" @click="onClick">
		<template #header>
			<div>
				<bonkxbt-heading tag="h2" bold :class="$style.cardHeading">
					{{ destination.label }}
				</bonkxbt-heading>
				<div :class="$style.cardDescription">
					<bonkxbt-text color="text-light" size="small">
		...
		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<div :class="$style.activeStatusText" data-test-id="destination-activator-status">
					<bonkxbt-text v-if="nodeParameters.enabled" :color="'success'" size="small" bold>
						{{ i18n.baseText('workflowActivator.active') }}
					</bonkxbt-text>
					<bonkxbt-text v-else color="text-base" size="small" bold>
				...
				<bonkxbt-action-toggle :actions="actions" theme="dark" @action="onAction" />
			</div>
		</template>
	</bonkxbt-card>
</template>
