...
import type { DatatableColumn } from 'bonkxbt-design-system';
...
<template>
	<PageViewLayout>
		...
		<div v-if="loading" class="resource-list-loading">
			<bonkxbt-loading :rows="25" :shrink-last="false" />
		</div>
		<template v-else>
			...
			<PageViewLayoutList v-else :overflow="type !== 'list'">
				<template #header>
					<div :class="$style['filters-row']">
						<div :class="$style.filters">
							...
							<div :class="$style['sort-and-filter']">
								<bonkxbt-select v-model="sortBy" data-test-id="resources-list-sort">
									<bonkxbt-option
					...
					<div v-if="showFiltersDropdown" v-show="hasFilters" class="mt-xs">
						<bonkxbt-info-tip :bold="false">
							...
							<bonkxbt-link data-test-id="workflows-filter-reset" size="small" @click="resetFilters">
				...
				<bonkxbt-text v-else color="text-base" size="medium" data-test-id="resources-list-empty">
