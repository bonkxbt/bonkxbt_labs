...
import type { ITelemetryTrackProperties } from 'bonkxbt-workflow';
	...
	>
			...
			<div v-else :class="$style.container">
				...
				<enterprise-edition :features="[EnterpriseEditionFeature.Sharing]" :class="$style.content">
					<div>
						...
						/>
						<bonkxbt-info-tip v-if="isHomeTeamProject" :bold="false" class="mt-s">
		...
		<template #footer>
			<div v-if="!isSharingEnabled" :class="$style.actionButtons">
				<bonkxbt-button @click="goToUpgrade">
					{{
			...
			>
				<bonkxbt-text v-show="isDirty" color="text-light" size="small" class="mr-xs">
					{{ i18n.baseText('workflows.shareModal.changesHint') }}
				</bonkxbt-text>
				<bonkxbt-button v-if="isHomeTeamProject" type="secondary" @click="modalBus.emit('close')">
