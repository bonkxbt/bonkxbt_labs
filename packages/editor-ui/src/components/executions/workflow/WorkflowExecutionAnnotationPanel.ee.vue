...
import type { AnnotationVote, ExecutionSummary } from 'bonkxbt-workflow';
import { useExecutionsStore } from '@/stores/executions.store';
import AnnotationTagsDropdown from '@/components/AnnotationTagsDropdown.ee.vue';
import { createEventBus } from 'bonkxbt-design-system';
	...
	>
		...
		<div :class="$style.section">
			<div :class="$style.heading">
				<bonkxbt-heading tag="h3" size="small" color="text-dark">
			...
			>
				...
				>
					<bonkxbt-text :class="$style.key" size="small" color="text-base">
						{{ attr }}
					</bonkxbt-text>
					<bonkxbt-text :class="$style.value" size="small" color="text-base">
						{{ activeExecution?.customData[attr] }}
					</bonkxbt-text>
				</div>
			</div>
			<div v-else :class="$style.noResultsContainer" data-test-id="execution-annotation-data-empty">
				<bonkxbt-text color="text-base" size="small" align="center">
					<span v-bonkxbt-html="i18n.baseText('executionAnnotationView.data.notFound')" />
				</bonkxbt-text>
			</div>
		</div>
	</div>
