...
<template>
	...
	>
		<div v-if="loading" :class="$style.loading">
			<bonkxbt-loading :rows="2" :shrink-last="false" :loading="loading" />
		</div>
		<div v-else-if="workflow">
			<bonkxbt-heading :bold="true" size="small">{{ workflow.name }}</bonkxbt-heading>
			<div v-if="!simpleView" :class="$style.content">
				...
				<bonkxbt-text v-if="workflow.user" size="small" color="text-light">
					{{
