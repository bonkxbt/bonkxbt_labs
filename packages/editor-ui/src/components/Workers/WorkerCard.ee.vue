...
import type { WorkerStatus } from '@bonkxbt/api-types';
...
<template>
	<bonkxbt-card v-if="worker" :class="$style.cardLink">
		<template #header>
			<bonkxbt-heading
				tag="h2"
				bold
		...
		<div :class="$style.cardDescription">
			<bonkxbt-text color="text-light" size="small" :class="$style.container">
				<span
					>{{ i18n.baseText('workerList.item.lastUpdated') }} {{ secondsSinceLastUpdateString }}s
					ago | bonkxbt-Version: {{ worker.version }} | Architecture: {{ worker.arch }} (
