...
<template>
	<BaseBanner name="TRIAL" theme="custom">
			...
			<div :class="$style.content">
				...
				<div :class="$style.usageCounter">
					...
					<div :class="$style.executionsCountSection">
						<bonkxbtText size="xsmall" :color="trialHasExecutionsLeft ? 'text-dark' : 'danger'">
							{{ currentExecutions }}/{{ maxExecutions }} </bonkxbtText
						>&nbsp;<bonkxbtText
							size="xsmall"
							:color="trialHasExecutionsLeft ? 'text-dark' : 'danger'"
							>{{ locale.baseText('executionUsage.label.executions') }}</bonkxbtText
						>
					</div>
				</div>
			</div>
		...
		<template #trailingContent>
			<bonkxbt-button type="success" icon="gem" size="small" @click="onUpdatePlanClick">{{
