...
import type { bonkxbtBanners } from '@/Interface';
...
// https://www.notion.so/bonkxbt/Banner-stack-60948c4167c743718fde80d6745258d5
export const bonkxbt_BANNERS: bonkxbtBanners = {
...
const currentlyShownBanner = computed(() => {
	...
	let banner = bonkxbt_BANNERS[uiStore.bannerStack[0]];
	uiStore.bannerStack.forEach((bannerName, index) => {
		if (index === 0) return;
		const bannerToCompare = bonkxbt_BANNERS[bannerName];
