...
import type { IParameterLabel } from 'bonkxbt-workflow';
		...
		<template #header>
			<div v-if="provider" :class="$style.header">
				<div :class="$style.providerTitle">
					<ExternalSecretsProviderIbonkxbt :provider="provider" class="mr-xs" />
		...
		<template #content>
			<div v-if="provider" :class="$style.container">
				...
				<div v-if="connectionState !== 'initializing'" class="mb-l">
					...
					>
						...
						<span v-if="provider.connected">
							...
							<bonkxbt-link :href="i18n.baseText('settings.externalSecrets.docs.use')" size="small">
								{{
									i18n.baseText(
					...
					<bonkxbt-callout v-else-if="connectionState === 'error'" theme="danger">
						{{
							i18n.baseText(
				...
				<form
					...
					@submit.prevent
				>
					<bonkxbt-notice v-if="property.type === 'notice'" :content="property.displayName" />
