...
import type { PublicInstalledPackage } from 'bonkxbt-workflow';
...
<template>
	<div :class="$style.cardContainer" data-test-id="community-package-card">
		<div v-if="loading" :class="$style.cardSkeleton">
			<bonkxbt-loading :class="$style.loader" variant="p" :rows="1" />
			<bonkxbt-loading :class="$style.loader" variant="p" :rows="1" />
		</div>
		<div v-else-if="communityPackage" :class="$style.packageCard">
			<div :class="$style.cardInfoContainer">
				<div :class="$style.cardTitle">
					<bonkxbt-text :bold="true" size="large">{{ communityPackage.packageName }}</bonkxbt-text>
				</div>
				<div :class="$style.cardSubtitle">
					<bonkxbt-text :bold="true" size="small" color="text-light">
						{{
					...
					<bonkxbt-text size="small" color="text-light">
			...
			<div :class="$style.cardControlsContainer">
				<bonkxbt-text :bold="true" size="large" color="text-light">
					v{{ communityPackage.installedVersion }}
				</bonkxbt-text>
				<bonkxbt-tooltip v-if="communityPackage.failedLoading === true" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.failedToLoad.tooltip') }}
						</div>
					</template>
					<bonkxbt-icon icon="exclamation-triangle" color="danger" size="large" />
				</bonkxbt-tooltip>
				<bonkxbt-tooltip v-else-if="communityPackage.updateAvailable" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.updateAvailable.tooltip') }}
						</div>
					</template>
					<bonkxbt-button outline label="Update" @click="onUpdateClick" />
				</bonkxbt-tooltip>
				<bonkxbt-tooltip v-else placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.upToDate.tooltip') }}
						</div>
					</template>
					<bonkxbt-icon icon="check-circle" color="text-light" size="large" />
				</bonkxbt-tooltip>
				<div :class="$style.cardActions">
					<bonkxbt-action-toggle :actions="packageActions" @action="onAction"></bonkxbt-action-toggle>
				</div>
			</div>
		</div>
	</div>
