...
import type { IMenuItem } from 'bonkxbt-design-system';
...
const sidebarMenuItems = computed<IMenuItem[]>(() => {
	const menuItems: IMenuItem[] = [
		...
		},
		{
			id: 'settings-api',
			icon: 'plug',
			label: i18n.baseText('settings.bonkxbtapi'),
			position: 'top',
...
<template>
	<div :class="$style.container">
		<bonkxbt-menu :items="sidebarMenuItems">
			<template #header>
				<div :class="$style.returnButton" data-test-id="settings-back" @click="emit('return')">
					<i class="mr-xs">
						<font-awesome-icon icon="arrow-left" />
					</i>
					<bonkxbt-heading size="large" :bold="true">{{ i18n.baseText('settings') }}</bonkxbt-heading>
				</div>
			</template>
			<template #menuSuffix>
				<div :class="$style.versionContainer">
					<bonkxbt-link size="small" @click="uiStore.openModal(ABOUT_MODAL_KEY)">
