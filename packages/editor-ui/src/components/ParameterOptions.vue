...
import type { INodeProperties, NodeParameterValueType } from 'bonkxbt-workflow';
...
<template>
	<div :class="$style.container" data-test-id="parameter-options-container">
		<div v-if="loading" :class="$style.loader" data-test-id="parameter-options-loader">
			<bonkxbt-text v-if="loading" size="small">
				<bonkxbt-icon icon="sync-alt" size="xsmall" :spin="true" />
				{{ loadingMessage }}
			</bonkxbt-text>
		</div>
