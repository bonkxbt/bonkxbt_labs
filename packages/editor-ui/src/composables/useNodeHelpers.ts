...
import { NodeHelpers, ExpressionEvaluatorProxy, NodeConnectionType } from 'bonkxbt-workflow';
import type {
	INodeProperties,
...
export function useNodeHelpers() {
					...
					(targetInputIndex: string) => {
						...
						if (endpoint?.endpoint) {
							...
							if (output && output.total > 0) {
								(endpoint.endpoint as bonkxbtPlusEndpoint).setSuccessOutput(
	...
	const SINGLE_EXECUTION_NODES: { [key: string]: { [key: string]: NodeParameterValue[] } } = {
		'bonkxbt-nodes-base.code': {
			mode: [undefined, 'runOnceForAllItems'],
		},
		'bonkxbt-nodes-base.executeWorkflow': {
			mode: [undefined, 'once'],
		},
		'bonkxbt-nodes-base.crateDb': {
			operation: [undefined, 'update'], // default insert
		},
		'bonkxbt-nodes-base.timescaleDb': {
			operation: [undefined, 'update'], // default insert
		},
		'bonkxbt-nodes-base.microsoftSql': {
			operation: [undefined, 'update', 'delete'], // default insert
		},
		'bonkxbt-nodes-base.questDb': {
			operation: [undefined], // default insert
		},
		'bonkxbt-nodes-base.mongoDb': {
			operation: ['insert', 'update'],
		},
		'bonkxbt-nodes-base.redis': {
