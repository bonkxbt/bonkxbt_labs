...
describe('usePageRedirectionHelper', () => {
	...
	beforeEach(() => {
		...
		const url = 'https://test.app.bonkxbt.cloud';
		...
		versionStore.setVersionNotificationSettings({
			...
			endpoint: '',
			infoUrl:
				'https://docs.bonkxbt.io/release-notes/#bonkxbt1652?utm_source=bonkxbt_app&utm_medium=instance_upgrade_releases',
		});
	});
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			'https://bonkxbt.io/pricing?utm_campaign=upgrade-api&source=advanced-permissions',
		],
		[
			'default',
			'development',
		...
		[
			...
			'production',
			ROLE.Owner,
			`https://app.bonkxbt.cloud/login?code=123&returnPath=${encodeURIComponent(
				'/account/change-plan',
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			`https://app.bonkxbt.cloud/login?code=123&returnPath=${encodeURIComponent('/manage')}`,
		],
		[
			'cloud',
			'production',
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			`https://app.bonkxbt.cloud/login?code=123&returnPath=${encodeURIComponent('/dashboard')}`,
		],
		['cloud', 'production', ROLE.Member, 'https://test.app.bonkxbt.cloud'],
	])(
