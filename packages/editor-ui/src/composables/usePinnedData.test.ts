...
import { NodeConnectionType, STICKY_NODE_TYPE, type INodeTypeDescription } from 'bonkxbt-workflow';
	...
	describe('canPinData()', () => {
		...
		it('does not allow pin without output', async () => {
			const node = ref({
				...
				typeVersion: 1,
				type: 'bonkxbt-nodes-base.stopAndError',
			} as INodeUi);
			getNodeType.mockReturnValue(makeNodeType([], 'bonkxbt-nodes-base.stopAndError'));
