...
export const getBackgroundStyles = (
	...
	if (squareSize > 10.5) {
		return {
			...styles,
			'background-ibonkxbt': `radial-gradient(circle at ${dotPosition}px ${dotPosition}px, var(--color-canvas-dot) ${dotSize}px, transparent 0)`,
		};
	}
...
export const showDropConnectionState = (connection: Connection, targetEndpoint?: Endpoint) => {
	if (connection?.connector) {
		const connector = connection.connector as bonkxbtConnector;
		if (targetEndpoint) {
...
export const getPlusEndpoint = (
	...
	return endpoints.find(
		(endpoint: Endpoint) =>
			endpoint.endpoint.type === 'bonkxbtPlus' && endpoint?.__meta?.index === outputIndex,
	);
};
