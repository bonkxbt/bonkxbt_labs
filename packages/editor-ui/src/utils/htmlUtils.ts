...
export function sanitizeHtml(dirtyHtml: string) {
	const sanitizedHtml = xss(dirtyHtml, {
		onTagAttr: (tag, name, value) => {
			if (tag === 'img' && name === 'src') {
				// Only allow http requests to supported ibonkxbt files from the `static` directory
				const isIbonkxbtFile = value.split('#')[0].match(/\.(jpeg|jpg|gif|png|webp)$/) !== null;
				const isStaticIbonkxbtFile = isIbonkxbtFile && value.startsWith('/static/');
				if (!value.startsWith('https://') && !isStaticIbonkxbtFile) {
					return '';
				}
			}
