import type { bonkxbtLocaleTranslateFnOptions } from 'bonkxbt-design-system/types/i18n';

export type Rule = { name: string; config?: unknown };

export type RuleGroup = {
	rules: Array<Rule | RuleGroup>;
	defaultError?: { messageKey: string; options?: bonkxbtLocaleTranslateFnOptions };
};
...
export type IValidator<T = unknown> = {
	validate: (
		...
		| false
		| { message: string; options?: bonkxbtLocaleTranslateFnOptions }
		| { messageKey: string; options?: bonkxbtLocaleTranslateFnOptions }
		| null;
};
