...
import { useI18n } from 'bonkxbt-design-system/composables/useI18n';
...
<template>
	<div :class="$style.container" data-test-id="code-diff-suggestion">
		...
		<div :class="$style.actions">
			<div v-if="error">
				<bonkxbt-icon icon="exclamation-triangle" color="danger" class="mr-5xs" />
			...
			<div v-else-if="replaced">
				...
				<bonkxbt-icon icon="check" color="success" class="ml-xs" />
			...
			<bonkxbt-button
				...
				:loading="replacing"
				@click="() => emit('replace')"
				>{{ replacing ? t('codeDiff.replacing') : t('codeDiff.replaceMyCode') }}</bonkxbt-button
			>
		</div>
	</div>
</template>
