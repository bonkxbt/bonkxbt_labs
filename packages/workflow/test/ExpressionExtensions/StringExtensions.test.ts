...
describe('Data Transformation Functions', () => {
	describe('String Data Transformation Functions', () => {
		...
		test('.isEmail should work on a string', () => {
			...
			expect(evaluate('={{ "test @ bonkxbt".isEmail() }}')).toEqual(false);
		});
		...
		test('.base64Encode should work on a string', () => {
			expect(evaluate('={{ "bonkxbt test".base64Encode() }}')).toBe('bjhuIHRlc3Q=');
		});

		test('.base64Decode should work on a string', () => {
			expect(evaluate('={{ "bjhuIHRlc3Q=".base64Decode() }}')).toBe('bonkxbt test');
		});
	});
});
