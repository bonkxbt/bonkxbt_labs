...
describe('generateNodesGraph', () => {
	test('should return node graph when node type is unknown', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['bonkxbt-nodes-base.manualTrigger', 'test.unknown'],
	...
	test('should return node graph when node has multiple operation fields with different display options', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['bonkxbt-nodes-base.manualTrigger', 'test.googleSheets'],
				node_connections: [{ start: '0', end: '1' }],
				nodes: {
					'0': {
						id: 'fa7d5628-5a47-4c8f-98ef-fb3532e5a9f5',
		...
		const workflow: IWorkflowBase = {
			...
			nodes: [
				...
				},
				{
					parameters: {
						content:
							"test\n\n## I'm a note \n**Double click** to edit me. [Guide](https://docs.bonkxbt.io/workflows/sticky-notes/)",
					},
					id: '03e85c3e-4303-4f93-8d62-e05d457e8f70',
					name: 'Sticky Note',
					type: 'bonkxbt-nodes-base.stickyNote',
					typeVersion: 1,
					position: [240, 140],
				},
			],
	...
	test('should return node graph indicating pinned data', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			...
			nodeGraph: {
				is_pinned: true,
				node_connections: [],
				node_types: ['bonkxbt-nodes-base.manualTrigger'],
				nodes: {
					'0': {
						id: 'e59d3ad9-3448-4899-9f47-d2922c8727ce',
						position: [460, 460],
	...
	test('should return graph with webhook node', () => {
			...
			nodes: [
				{
					...
					name: 'Webhook',
					type: 'bonkxbt-nodes-base.webhook',
					typeVersion: 1.1,
					position: [520, 380],
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['bonkxbt-nodes-base.webhook'],
				node_connections: [],
				nodes: {
					'0': {
						id: '5e49e129-2c59-4650-95ea-14d4b94db1f3',
	...
	test('should return graph with http v4 node with generic auth', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['bonkxbt-nodes-base.httpRequest'],
				node_connections: [],
				nodes: {
					'0': {
						id: '04d6e44f-09c1-454d-9225-60aeed7f022c',
	...
	test('should support custom connections like in AI nodes', () => {
		const workflow: Partial<IWorkflowBase> = {
			nodes: [
				...
				},
				{
					...
					name: 'Chain',
					type: '@bonkxbt/bonkxbt-nodes-langchain.chainLlm',
					typeVersion: 1,
					position: [760, 320],
				},
				{
					...
					name: 'Model',
					type: '@bonkxbt/bonkxbt-nodes-langchain.lmChatOpenAi',
					typeVersion: 1,
					position: [780, 500],
				},
			],
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: [
					'bonkxbt-nodes-base.manualTrigger',
					'@bonkxbt/bonkxbt-nodes-langchain.chainLlm',
					'@bonkxbt/bonkxbt-nodes-langchain.lmChatOpenAi',
				],
				node_connections: [
					{
						start: '0',
	...
	test('should add run and items count', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes, { runData })).toEqual({
			...
			nodeGraph: {
				...
				node_types: [
					'bonkxbt-nodes-base.executeWorkflowTrigger',
					'bonkxbt-nodes-base.set',
					'bonkxbt-nodes-base.set',
					'bonkxbt-nodes-base.set',
					'bonkxbt-nodes-base.debugHelper',
					'bonkxbt-nodes-base.switch',
				],
				nodes: {
					'0': {
					...
					'5': {
						...
						position: [1680, 640],
						runs: 2,
						type: 'bonkxbt-nodes-base.switch',
						version: 3.2,
					},
				},
				notes: {},
