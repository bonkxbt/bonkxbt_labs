...
import type { LogScope } from '@bonkxbt/config';
...
export type BinaryFileType = 'text' | 'json' | 'ibonkxbt' | 'audio' | 'video' | 'pdf' | 'html';
...
export namespace DeclarativeRestApiSettings {
	...
	export type ResultOptions = {
		...
		requestOperations?: IbonkxbtRequestOperations;
	};
}
...
export type IExecuteResponsePromiseData = IDataObject | IbonkxbtHttpFullResponse;
...
export type IbonkxbtHttpResponse = IDataObject | Buffer | GenericValue | GenericValue[] | null;

export interface IbonkxbtHttpFullResponse {
	body: IbonkxbtHttpResponse | Readable;
...
export interface IbonkxbtRequestOperations {
	pagination?:
		| IbonkxbtRequestOperationPaginationGeneric
		| IbonkxbtRequestOperationPaginationOffset
		| ((
...
export interface IbonkxbtRequestOperationPaginationBase {
	type: string;
	properties: {
		[key: string]: unknown;
	};
...
export interface IbonkxbtRequestOperationPaginationGeneric extends IbonkxbtRequestOperationPaginationBase {
	type: 'generic';
	properties: {
		continue: boolean | string;
		request: IRequestOptionsSimplifiedAuth;
...
export interface IbonkxbtRequestOperationPaginationOffset extends IbonkxbtRequestOperationPaginationBase {
	type: 'offset';
	properties: {
...
namespace ExecuteFunctions {
	...
	export type GetNodeParameterFn = {
		// @TECH_DEBT: Refactor to remove this barely used overload - bonkxbt-5632
...
export interface ILoadOptions {
	routing?: {
		operations?: IbonkxbtRequestOperations;
...
export interface INodePropertyRouting {
	operations?: IbonkxbtRequestOperations; // Should be changed, does not sound right
	output?: INodeRequestOutput;
	request?: DeclarativeRestApiSettings.HttpRequestOptions;
...
export type NodeDefaults = Partial<{
	/**
	 * @deprecated Use {@link INodeTypeBaseDescription.iconColor|iconColor} instead. `iconColor` supports dark mode and uses preset colors from bonkxbt's design system.
	 */
	color: string;
	name: string;
}>;
...
export interface INodeGraphItem {
	...
	agent?: string; //@bonkxbt/bonkxbt-nodes-langchain.agent
	...
	workflow_id?: string; //@bonkxbt/bonkxbt-nodes-langchain.toolWorkflow and bonkxbt-nodes-base.executeWorkflow
	runs?: number;
	items_total?: number;
}
...
export type bonkxbtAIProviderType = 'openai' | 'unknown';
...
export type IPersonalizationSurveyAnswersV4 = {
	...
	personalization_survey_bonkxbt_version: string;
