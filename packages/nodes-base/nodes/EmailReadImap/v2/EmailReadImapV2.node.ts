import type { ImapSimple, ImapSimpleOptions, Message, MessagePart } from '@bonkxbt/imap';
import { connect as imapConnect, getParts } from '@bonkxbt/imap';
...
import { NodeConnectionType, NodeOperationError, TriggerCloseError } from 'bonkxbt-workflow';
			...
			for (const attachmentPart of attachmentParts) {
				attachmentPromise = imapConnection
					...
					.then(async (partData) => {
						...
						// Return it in the format bonkxbt expects
				...
				onMail: async () => {
					if (connection) {
						...
						try {
							...
							// Wait with resolving till the returnedPromise got resolved, else bonkxbt will be unhappy
