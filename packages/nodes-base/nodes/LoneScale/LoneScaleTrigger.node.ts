		...
		properties: [
			{
				...
				default: '',
				// eslint-disable-next-line bonkxbt-nodes-base/node-param-description-missing-final-period, bonkxbt-nodes-base/node-param-description-wrong-for-dynamic-options
		...
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				const webhook = await lonescaleApiRequest.call(
					this,
					'GET',
					`/workflows/${workflowId}/hook?type=bonkxbt`,
				);
			...
			async delete(this: IHookFunctions): Promise<boolean> {
				...
				try {
					await lonescaleApiRequest.call(
						this,
						'DELETE',
						`/workflows/${webhookData.webhookId}/hook?type=bonkxbt`,
					);
				} catch (error) {
					return false;
				}
