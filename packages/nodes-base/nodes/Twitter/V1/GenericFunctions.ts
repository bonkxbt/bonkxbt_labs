...
import { NodeApiError, NodeOperationError, sleep } from 'bonkxbt-workflow';
...
export async function uploadAttachments(
	...
	for (const binaryPropertyName of binaryProperties) {
		...
		const isIbonkxbt = binaryData.mimeType.includes('ibonkxbt');

		if (isIbonkxbt && isAnimatedWebp) {
			throw new NodeOperationError(
				this.getNode(),
				'Animated .webp ibonkxbts are not supported use .gif instead',
				{ itemIndex: i },
			);
		}
