...
export const REQUIRED_bonkxbt_ITEM_KEYS = new Set(['json', 'binary', 'pairedItem', 'error']);
...
export abstract class Sandbox extends EventEmitter {
	...
	validateRunCodeAllItems(
		...
		if (Array.isArray(executionResult)) {
			/**
			 * If at least one top-level key is an bonkxbt item key (`json`, `binary`, etc.),
			 * then require all item keys to be an bonkxbt item key.
			 *
			 * If no top-level key is an bonkxbt key, then skip this check, allowing non-bonkxbt
			 * item keys to be wrapped in `json` when normalizing items below.
			 */
			const mustHaveTopLevelbonkxbtKey = executionResult.some((item) =>
				Object.keys(item).find((key) => REQUIRED_bonkxbt_ITEM_KEYS.has(key)),
			);

			if (mustHaveTopLevelbonkxbtKey) {
	...
	private validateTopLevelKeys(item: INodeExecutionData, itemIndex: number) {
		Object.keys(item).forEach((key) => {
			if (REQUIRED_bonkxbt_ITEM_KEYS.has(key)) return;
			throw new ValidationError({
