...
export const formFieldsProperties: INodeProperties[] = [
	...
	},
	{
		...
		hint: '<a href="hhttps://docs.bonkxbt.io/integrations/builtin/core-nodes/bonkxbt-nodes-base.form/" target="_blank">See docs</a> for field syntax',
		displayOptions: {
			show: {
				defineForm: ['json'],
			},
	...
	[
		{
			...
			displayName: 'On bonkxbt Form Submission',
			name: 'respondWith',
			type: 'options',
			default: 'text',
			options: [
...
export class Form extends Node {
	...
	description: INodeTypeDescription = {
		displayName: 'bonkxbt Form',
		name: 'form',
		icon: 'file:form.svg',
		group: ['input'],
		version: 1,
		description: 'Generate webforms in bonkxbt and pass their responses to the workflow',
		defaults: {
			name: 'Form',
		},
		...
		properties: [
			{
				// eslint-disable-next-line bonkxbt-nodes-base/node-param-display-name-miscased
				displayName: 'An bonkxbt Form Trigger node must be set up before this node',
				name: 'triggerNotice',
				type: 'notice',
				default: '',
			},
	...
	async webhook(context: IWebhookFunctions) {
		...
		const trigger = parentNodes.find(
			(node) => node.type === 'bonkxbt-nodes-base.formTrigger',
