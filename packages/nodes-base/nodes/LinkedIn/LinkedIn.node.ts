			...
			try {
				if (resource === 'post') {
					if (operation === 'create') {
						...
						if (shareMediaCategory === 'Ibonkxbt') {
							if (additionalFields.title) {
								title = additionalFields.title as string;
							}
							// Send a REQUEST to prepare a register of a media ibonkxbt file
							...
							const registerObject = await linkedInApiRequest.call(
								this,
								'POST',
								'/ibonkxbts?action=initializeUpload',
								registerRequest,
							);

							const binaryPropertyName = this.getNodeParameter('binaryPropertyName', i);
							const ibonkxbtMetadata = this.helpers.assertBinaryData(i, binaryPropertyName);

							const buffer = await this.helpers.getBinaryDataBuffer(i, binaryPropertyName);
							const { uploadUrl, ibonkxbt } = registerObject.value;

							const headers = {};
							Object.assign(headers, { 'Content-Type': ibonkxbtMetadata.mimeType });
							...
							Object.assign(body, ibonkxbtBody);
							...
							if (additionalFields.thumbnailBinaryPropertyName) {
								...
								Object.assign(articleBody.content.article, { thumbnail: ibonkxbt });
							}
