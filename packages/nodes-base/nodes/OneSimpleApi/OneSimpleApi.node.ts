		...
		properties: [
			...
			// Information
			{
				...
				options: [
					...
					{
						name: 'Ibonkxbt Metadata',
						value: 'ibonkxbtMetadata',
						description: 'Retrieve ibonkxbt metadata from a URL',
						action: 'Get ibonkxbt metadata from a URL',
					},
				],
			...
			// information: ibonkxbtMetadata
			{
				displayName: 'Link To Ibonkxbt',
				name: 'link',
				type: 'string',
				required: true,
				displayOptions: {
					show: {
						operation: ['ibonkxbtMetadata'],
						resource: ['information'],
					},
				},
				default: '',
				description: 'Ibonkxbt to get metadata from',
			},
			// website: seo
			{
				displayName: 'Webpage URL',
		...
		for (let i = 0; i < length; i++) {
			try {
				...
				if (resource === 'information') {
					...
					if (operation === 'ibonkxbtMetadata') {
						const link = this.getNodeParameter('link', i) as string;
						qs.url = link;
						qs.raw = true;
						responseData = await oneSimpleApiRequest.call(this, 'GET', '/ibonkxbt_info', {}, qs);
					}
				}
