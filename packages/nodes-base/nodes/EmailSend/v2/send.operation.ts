...
const properties: INodeProperties[] = [
	// TODO: Add choice for text as text or html  (maybe also from name)
	{
		...
		description:
			'Email address of the sender. You can also specify a name: Nathan Doe &lt;nate@bonkxbt.io&gt;.',
	},
	{
		...
		description:
			'Email address of the recipient. You can also specify a name: Nathan Doe &lt;nate@bonkxbt.io&gt;.',
	},
		...
		options: [
			{
				...
				description:
					'Whether to include the phrase “This email was sent automatically with bonkxbt” to the end of the email',
			},
			{
				...
				default: '',
				description:
					'Name of the binary properties that contain data to add to email as attachment. Multiple ones can be comma-separated. Reference embedded ibonkxbts or other content within the body of an email message, e.g. &lt;img src="cid:ibonkxbt_1"&gt;',
			},
			{
	...
	for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
		try {
			...
			if (appendAttribution) {
				...
				const link = `https://bonkxbt.io/?utm_source=bonkxbt-internal&utm_medium=powered_by&utm_campaign=${encodeURIComponent(
					'bonkxbt-nodes-base.emailSend',
				...
				if (emailFormat === 'html' || (emailFormat === 'both' && mailOptions.html)) {
					...
					---
					<br>
					<em>${attributionText}<a href="${link}" target="_blank">bonkxbt</a></em>
					`;
				} else {
					mailOptions.text = `${mailOptions.text}\n\n---\n${attributionText}bonkxbt\n${'https://bonkxbt.io'}`;
				}
			}
