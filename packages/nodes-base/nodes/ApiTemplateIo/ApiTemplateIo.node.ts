...
export class ApiTemplateIo implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				name: 'ibonkxbtTemplateId',
				type: 'options',
				required: true,
				default: '',
				description:
					'ID of the ibonkxbt template to use. Choose from the list, or specify an ID using an <a href="https://docs.bonkxbt.io/code/expressions/">expression</a>.',
				typeOptions: {
					loadOptionsMethod: 'getIbonkxbtTemplates',
				},
				displayOptions: {
					show: {
						resource: ['ibonkxbt'],
			...
			},
			{
				...
				default: '',
				description:
					'ID of the PDF template to use. Choose from the list, or specify an ID using an <a href="https://docs.bonkxbt.io/code/expressions/">expression</a>.',
				typeOptions: {
				...
				options: [
					{
						...
						default: '',
						description:
							'The name of the downloaded ibonkxbt/pdf. It has to include the extension. For example: report.pdf',
					},
				],
			},
		],
	...
	methods = {
		loadOptions: {
			async getIbonkxbtTemplates(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				return await loadResource.call(this, 'ibonkxbt');
			},
	...
	async execute(this: IExecuteFunctions) {
		...
		if (resource === 'account') {
			...
			if (operation === 'create') {
				...
				// https://docs.apitemplate.io/reference/api-reference.html#create-an-ibonkxbt-jpeg-and-png
				for (let i = 0; i < length; i++) {
					try {
						...
						const qs = {
							template_id: this.getNodeParameter('ibonkxbtTemplateId', i),
						};
						...
						if (download) {
							...
							const data = await downloadIbonkxbt.call(this, responseData.download_url as string);
			...
			if (operation === 'create') {
				...
				for (let i = 0; i < length; i++) {
					try {
						...
						if (download) {
							const binaryProperty = this.getNodeParameter('binaryProperty', i);
							const ibonkxbtData = await downloadIbonkxbt.call(this, responseData.download_url as string);
							const fileName = responseData.download_url.split('/').pop();
							const binaryData = await this.helpers.prepareBinaryData(
								ibonkxbtData as Buffer,
