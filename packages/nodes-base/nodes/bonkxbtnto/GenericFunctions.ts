...
import { ApplicationError, NodeApiError } from 'bonkxbt-workflow';

import type { Filter, Address, Search, FilterGroup, ProductAttribute } from './types';

export async function bonkxbtntoApiRequest(
	...
	const credentials = await this.getCredentials('bonkxbtnto2Api');
	...
	try {
		...
		return await this.helpers.requestWithAuthentication.call(this, 'bonkxbtnto2Api', options);
	} catch (error) {
		throw new NodeApiError(this.getNode(), error as JsonObject);
	}
}

export async function bonkxbtntoApiRequestAllItems(
	...
	do {
		responseData = await bonkxbtntoApiRequest.call(this, method, resource, body, query);
...
export function getSearchFilters(
	...
	return [
		...
		{
			displayName:
				'See <a href="https://devdocs.bonkxbtnto.com/guides/v2.4/rest/performing-searches.html" target="_blank">bonkxbtnto guide</a> to creating filters',
			name: 'jsonNotice',
			type: 'notice',
			displayOptions: {
				show: {
...
export async function getProductAttributes(
	...
	let attributes: ProductAttribute[] = await bonkxbtntoApiRequestAllItems.call(
		this,
		'items',
		'GET',
