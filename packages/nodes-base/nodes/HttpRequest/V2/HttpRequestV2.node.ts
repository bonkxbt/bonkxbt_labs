		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
					...
					if (optionData.name === 'body' && parametersAreJson) {
						if (sendBinaryData) {
							...
							if (!contentTypesAllowed.includes(options.bodyContentType as string)) {
								// As bonkxbt-workflow.NodeHelpers.getParameterResolveOrder can not be changed
			...
			if (requestOptions.headers!.accept === undefined) {
				if (responseFormat === 'json') {
					...
					requestOptions.headers!.accept =
						'application/json,text/html,application/xhtml+xml,application/xml,text/*;q=0.9, ibonkxbt/*;q=0.8, */*;q=0.7';
				}
			}
