...
import { ApplicationError, ICredentialsHelper, NodeHelpers, WorkflowHooks } from 'bonkxbt-workflow';
...
export function createTemporaryDir(prefix = 'bonkxbt') {
...
export function setup(testData: WorkflowTestData[] | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		const loadInfo = knownNodes[nodeName.replace('bonkxbt-nodes-base.', '')];
		if (!loadInfo) {
			throw new UnrecognizedNodeTypeError('bonkxbt-nodes-base', nodeName);
		}
		const sourcePath = loadInfo.sourcePath.replace(/^dist\//, './').replace(/\.js$/, '.ts');
		const nodeSourcePath = path.join(baseDir, sourcePath);
