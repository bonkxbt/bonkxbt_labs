...
describe('WorkflowExecute', () => {
	...
	describe('runPartialWorkflow2', () => {
		...
		test("deletes dirty nodes' run data", async () => {
			...
			const trigger = createNodeData({ name: 'trigger', type: 'bonkxbt-nodes-base.manualTrigger' });
		...
		test('passes filtered run data to `recreateNodeExecutionStack`', async () => {
			...
			const trigger = createNodeData({ name: 'trigger', type: 'bonkxbt-nodes-base.manualTrigger' });
			const loop = createNodeData({ name: 'loop', type: 'bonkxbt-nodes-base.splitInBatches' });
