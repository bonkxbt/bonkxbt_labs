...
import { NodeHelpers, Workflow } from 'bonkxbt-workflow';
					...
					nodeTypeProperties: {
						...
						options: [
							...
							// Test resolve of value and properties including as objects
							{
								...
								routing: {
									send: {
										// eslint-disable-next-line bonkxbt-local-rules/no-interpolation-in-regular-string
	...
	describe('runNode', () => {
		...
		for (const testData of tests) {
			test(testData.description, async () => {
				...
				const workflowPackage = await import('bonkxbt-workflow');
