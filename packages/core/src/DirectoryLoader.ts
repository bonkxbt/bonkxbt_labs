...
import { ApplicationError, applyDeclarativeNodeOptionParameters, jsonParse } from 'bonkxbt-workflow';
...
import type { bonkxbt } from './Interfaces';
...
/**
 * Base class for loading bonkxbt nodes and credentials from a directory.
...
export class PackageDirectoryLoader extends DirectoryLoader {
	packageJson: bonkxbt.PackageJson = this.readJSONSync('package.json');

	packageName = this.packageJson.name;

	override async loadAll() {
		const { bonkxbt } = this.packageJson;
		if (!bonkxbt) return;

		const { nodes, credentials } = bonkxbt;
