jest.mock('bonkxbt-workflow', () => ({
	...jest.requireActual('bonkxbt-workflow'),
	LoggerProxy: { init: jest.fn() },
}));

import type { GlobalConfig } from '@bonkxbt/config';
import { mock } from 'jest-mock-extended';
import { LoggerProxy } from 'bonkxbt-workflow';
			...
			const globalConfig = mock<GlobalConfig>({
				logging: {
					...
					file: {
						fileSizeMax: 100,
						fileCountMax: 16,
						location: 'logs/bonkxbt.log',
					},
				},
			});

			const logger = new Logger(globalConfig, mock<InstanceSettingsConfig>({ bonkxbtFolder: '/tmp' }));
