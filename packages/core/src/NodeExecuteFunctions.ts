...
} from '@bonkxbt/client-oauth2';
import { ClientOAuth2 } from '@bonkxbt/client-oauth2';
...
import {
	...
	BLOCK_FILE_ACCESS_TO_bonkxbt_FILES,
	CONFIG_FILES,
...
export async function parseRequestObject(requestObject: IRequestOptions) {
	...
	// We are not using bonkxbt's interface as it would
...
// eslint-disable-next-line complexity
function convertbonkxbtRequestToAxios(bonkxbtRequest: IHttpRequestOptions): AxiosRequestConfig {
	// Destructure properties with the same name first.
	const { headers, method, timeout, auth, proxy, url } = bonkxbtRequest;
	...
	axiosRequest.params = bonkxbtRequest.qs;

	if (bonkxbtRequest.abortSignal) {
		axiosRequest.signal = bonkxbtRequest.abortSignal;
	}

	if (bonkxbtRequest.baseURL !== undefined) {
		axiosRequest.baseURL = bonkxbtRequest.baseURL;
	}

	if (bonkxbtRequest.disableFollowRedirect === true) {
		axiosRequest.maxRedirects = 0;
	}

	if (bonkxbtRequest.encoding !== undefined) {
		axiosRequest.responseType = bonkxbtRequest.encoding;
	}

	const host = getHostFromRequestObject(bonkxbtRequest);
	const agentOptions: AgentOptions = {};
	if (host) {
		agentOptions.servername = host;
	}
	if (bonkxbtRequest.skipSslCertificateValidation === true) {
		agentOptions.rejectUnauthorized = false;
	}
	...
	if (bonkxbtRequest.arrayFormat !== undefined) {
		axiosRequest.paramsSerializer = (params) => {
			return stringify(params, { arrayFormat: bonkxbtRequest.arrayFormat });
		};
	}

	const { body } = bonkxbtRequest;
	if (body) {
		...
		if (existingContentTypeHeaderKey === undefined) {
			...
			axiosRequest.data = new URLSearchParams(bonkxbtRequest.body as Record<string, string>);
		}
	...
	if (bonkxbtRequest.ignoreHttpStatusErrors) {
...
): Promise<IbonkxbtHttpFullResponse | IbonkxbtHttpResponse> {
	removeEmptyBody(requestOptions);

	const axiosRequest = convertbonkxbtRequestToAxios(requestOptions);
	if (
...
/**
 * Takes a buffer and converts it into the format bonkxbt uses. It encodes the binary data as
...
/**
 * Takes generic input data and brings it into the json format bonkxbt uses.
 *
...
/**
 * Takes generic input data and brings it into the new json, pairedItem format bonkxbt uses.
...
export const getRequestHelperFunctions = (
	...
	return {
		...
		async requestWithAuthenticationPaginated(
			...
			let tempResponseData: IbonkxbtHttpFullResponse;
			...
			const additionalKeys: IWorkflowDataProxyAdditionalKeys = {
				...
				$response: {} as IbonkxbtHttpFullResponse,
			...
			do {
				...
				const newResponse: IbonkxbtHttpFullResponse = Object.assign(
					{
						body: {},
						headers: {},
						statusCode: 0,
				...
				let contentBody: Exclude<IbonkxbtHttpResponse, Buffer>;
...
export function isFilePathBlocked(filePath: string): boolean {
	...
	const blockFileAccessTobonkxbtFiles = process.env[BLOCK_FILE_ACCESS_TO_bonkxbt_FILES] !== 'false';
	...
	//restrict access to .bonkxbt folder, ~/.cache/bonkxbt/public, and other .env.examples config related paths
	if (blockFileAccessTobonkxbtFiles) {
		const { bonkxbtFolder, staticCacheDir } = Container.get(InstanceSettings);
		const restrictedPaths = [bonkxbtFolder, staticCacheDir];
...
export const getFileSystemHelperFunctions = (node: INode): FileSystemHelperFunctions => ({
	...
	getStoragePath() {
		return path.join(Container.get(InstanceSettings).bonkxbtFolder, `storage/${node.type}`);
	},
